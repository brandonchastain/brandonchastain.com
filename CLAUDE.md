...
There is no build, test, or lint tooling. Validate changes by opening HTML files in a browser, or via the SWA preview deployment created on PRs to `master`. For local previewing, `start-local-server.bat` is available.
